...
if ($_POST['action'] == 'searchData') {
    ...
    $new = array_filter($result, function ($value) use ($search, &$srno) {
        ...
        if (strpos(strtolower($value['name']), strtolower($search)) !== FALSE || strpos(strtolower($value['email']), strtolower($search)) !== FALSE || strpos(strtolower($value['country_Name']), strtolower($search)) !== FALSE || strpos(strtolower($value['state_Name']), strtolower($search)) !== FALSE || strpos(strtolower($value['city_Name']), strtolower($search)) !== FALSE || strpos(strtolower($value['phone_Number']), strtolower($search)) !== FALSE) {
            ...
            $imgpath = SITE_URL . 'PHPOPS/Day19/Assignments/Project1/admin/assets/img/upload/' . $value['emp_Id'] . '/' . $value['profile_Pic'];
            if ($value['gender'] == "M") {
                $gender = 'Male';
            }
            if ($value['gender'] == "F") {
        ...
        <div class='col'><a href='" . SITE_URL . "PHPOPS/Day19/Assignments/Project1/admin/addEmployeeHtml.php?emp_Id="  . $value['emp_Id'] . "' type='button' id='udp"  . $value['emp_Id'] . "' class='udp btn btn-success upd'>UPDATE</a></div>
        ...
        <main class='position-relative my-3'>
            <div class="row d-flex justify-content-between mb-3">
                ...
                <div class="col-4 d-flex justify-content-end">
                    <a class="btn btn-outline-success" href="<?php echo SITE_URL; ?>PHPOPS/Day19/Assignments/Project1/admin/addEmployeeHtml.php" role="button">Add Employee</a>
                </div>
