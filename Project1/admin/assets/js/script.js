...
$(document).ready(function () {
  ...
  $("#userLogin").click(() => {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response == 'success') {
          window.location.href = 'http://ops.localhost//PHPOPS/Day19/Assignments/Project1/';
        }
        else {
          $('#alert').html('<span class="text-danger">Invalid Username or Password</span>');
        }
  ...
  $("#userLogout").click(() => {
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response == 'logout') {
          window.location.href = 'http://ops.localhost//PHPOPS/Day19/Assignments/Project1/index.php';
        }
      }
    });
  });
